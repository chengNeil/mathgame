...
let ranNum= ()=>{
    const addEqu = document.querySelector("#addEqu");
    ...
    addEqu.innerHTML=`${randomOne}+${randomTwo}`;
...
var equation = document.getElementById('addEqu');
...
let missCount = () =>{
  missNum++;
  missedTotal.innerHTML=`${missNum}`;

  localStorage.setItem("getMiss", missNum);
  ranNum();
  myScore=parseInt(hitNum)-parseInt(missNum);
  if (myScore<0) {
    myScore=0;
  }
  netScore.innerHTML=myScore;
  if (missedTotal.innerHTML >=6) {
    localStorage.setItem("level3Miss", 0);
    localStorage.setItem("getSubMiss", 0);
    localStorage.setItem("level3Hit", 0);
    localStorage.setItem("getSubHit", 0);
    localStorage.setItem("FinalScore", myScore);
    location.replace("summary.html");
  }
  localStorage.setItem("getScores", myScore);
}
let hitCount = () =>{
  hitNum++;
  hitsTotal.innerHTML=`${hitNum}`;
  localStorage.setItem("getHit", hitNum);
  ranNum();
  myScore=parseInt(hitNum)-parseInt(missNum);
  if (myScore<0) {
    myScore=0;
