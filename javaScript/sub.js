...
let ranNum= ()=>{
    const subEqu = document.querySelector("#subEqu");
    ...
    subEqu.innerHTML=`${randomOne}-${randomTwo}`;
    ...
    netScore.innerHTML=localStorage.getItem("getScores");
}
...
var equation = document.getElementById('subEqu');
...
let missCount = () =>{
  missNum++;
  ranNum();
  missedTotal.innerHTML=parseInt(missNum);
  hitsTotal.innerHTML=parseInt(hitNum);
  mySubScore= parseInt(netScore.innerHTML) + parseInt(hitNum)-parseInt(missNum);
  if (mySubScore<0) {
    mySubScore=0;
  }
  netScore.innerHTML=mySubScore;
  localStorage.setItem("getSubHit", hitsTotal.innerHTML);
  localStorage.setItem("getSubMiss", missedTotal.innerHTML);
  localStorage.setItem("getSubScores", netScore.innerHTML);
  if (missedTotal.innerHTML >=6) {
    localStorage.setItem("level3Miss", 0);
    localStorage.setItem("level3Hit", 0);
